...
@pytest.fixture
def engine_service(mock_eval_run_repo, mock_task_repo, mock_retrieval_service, mock_llm_service):
...
@pytest.mark.asyncio
async def test_execute_run_success(engine_service, mock_eval_run_repo, mock_task_repo, mock_llm_service):
    run_id = uuid.uuid4()
    ...
    assert final_call_kwargs["latency_ms"] == 500
    assert final_call_kwargs["total_tokens"] == 70
